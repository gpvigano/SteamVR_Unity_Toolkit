namespace VRTK
{
    ...
    /// <summary>
    /// The `[GearVRSimulator_CameraRig]` prefab is a mock Camera Rig set up that can be used to develop with VRTK without the need for VR Hardware.
    /// </summary>
    /// <remarks>
    ...
    public class SDK_InputGearVRSimulator : MonoBehaviour
    {
        ...
        [Header("Movement Key Binding Settings (not available on GearVR)")]
        ...
        /// <summary>
        /// The FindInScene method is used to find the `[GearVRSimulator_CameraRig]` GameObject within the current scene.
        /// </summary>
        /// <returns>Returns the found `[GearVRSimulator_CameraRig]` GameObject if it is found. If it is not found then it prints a debug log error.</returns>
        public static GameObject FindInScene()
        {
            if (cachedCameraRig == null && !destroyed)
            {
                ...
                if (!cachedCameraRig)
                {
                    VRTK_Logger.Error(VRTK_Logger.GetCommonMessage(VRTK_Logger.CommonMessageKeys.REQUIRED_COMPONENT_MISSING_FROM_SCENE, "[GearVRSimulator_CameraRig]", "SDK_InputGearVRSimulator", ". check that the `VRTK/Prefabs/CameraRigs/[GearVRSimulator_CameraRig]` prefab been added to the scene."));
                }
            }
